...
def compute_del(beta, rho):
    del_ = 2*beta**2*rho*(rho+2)
    print(f"Computed δ: {del_}")
    return del_

# compute gamma/beta when alpha = 0
def compute_critical_ratio(beta, rho):
    del_ = compute_del(beta, rho)
    crit_ratio = beta * np.sqrt((del_-1)/(del_*(beta**2+1)-1))
    print(f"Computed critical gamma/beta ratio: {crit_ratio}")
    return crit_ratio

beta, rho = compute_beta_for_cart_pole()
beta = 70.6
rho = 1e-8
ratio = compute_critical_ratio(beta, rho)

# check
gamma = beta * ratio
compute_alpha(beta, rho, gamma)

# pick new ratio
new_ratio = 950
gamma = beta * new_ratio
alpha = compute_alpha(beta, rho, gamma)
print(f"gamma^2/alpha = {gamma**2/alpha}")